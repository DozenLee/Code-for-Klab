...
class net:
    ...
    def loadData(self,samplePath=None):
        ...
        self.shuffle()
    # ------------------------------------------------------------------- #
    # picRead(self,dirpath=None):
    # 内容 : dirpathの画像を読み込んで
    #　　　　ndarrayに変更して
    #　　　　self.data/labelに置く
    # 引数 : self,dirpath  画像ファイル
    # 戻り値: なし
    # ------------------------------------------------------------------- #
    ...
    def train(self,epochNum=100,batchNum=1,finalLoss=1e-5):
        ...
        if trainNum<batchNum:
            raise Exception('sample too few,or use a smaller batch size')
